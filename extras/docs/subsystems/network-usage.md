...
English | [中文](network-usage.zh.md)

The optional [network-usage package](../../packages/experimental/network-usage/README.md) owns per-Session transfer accounting, spending consent, and metered proxy access. This reference lists its Host services. `TrafficOwner` identifies a Session or the literal `host` for requests without an Agent initiator.
