...
已认证的 Host 接口提供随包默认壁纸，并允许浏览器私有缓存。Client 注册设置分区及可卸载的主题覆盖。侧栏边框和中性底色区分各区域，不使用模糊。欢迎页输入区的外层保持透明。已有对话的输入区、菜单、子代理下拉列表、摘要浮层、弹窗、轨迹详情、代码块标题栏和全屏面板绘制当前壁纸副本，遮住下层文字。原生单选控件打开与壁纸一致的网页内列表，支持鼠标和键盘选择。皮肤不修改原有主题偏好。浏览器存储是偏好的唯一持有方，因此没有独立的运行时 invariant 配套模块。
